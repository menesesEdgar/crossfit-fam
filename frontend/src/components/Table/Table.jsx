          ...
          <T.Head className="text-nowrap">
            {columns?.map((col) => (
              <T.HeadCell
                ...
                scope="col"
                className={`${
                  col?.order || col?.id === "checkbox"
                    ? ""
                    : "pointer-events-none"
                } p-4 bg-crossfit-primary text-white ${
                  col?.id !== "checkbox"
                    ? "cursor-pointer hover:bg-crossfit-light-purple hover:text-white transition-colors ease-in-out duration-100"
                    : ""
                } ${col?.id === sortedBy && "bg-crossfit-light-purple"} ${
                  col?.classes
                }`}
